...
class MAC(object):
    ...
    def _init_critic(self):
        for agent in self.agents:
            agent.critic_net = Critic(self._get_input_shape(agent.idx), 1, self.c_mlp_layer_size, self.c_rnn_layer_size).to(self.device)
            agent.critic_tgt_net = Critic(self._get_input_shape(agent.idx), 1, self.c_mlp_layer_size, self.c_rnn_layer_size).to(self.device)
