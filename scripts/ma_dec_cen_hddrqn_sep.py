...
from macro_marl.MA_dec_cen_hddrqn_sep.team import Team_RNN
from macro_marl.MA_dec_cen_hddrqn_sep.utils.memory import ReplayMemory_rand, ReplayMemory_epi
from macro_marl.MA_dec_cen_hddrqn_sep.utils.utils import save_check_point
from macro_marl.MA_dec_cen_hddrqn_sep.learning_methods import QLearn_squ_dec_cen_0
...
def train(env_id, obs_one_hot, target_flick_prob, agent_trans_noise, target_rand_move, 
          ...
          h0_speed_ps, h1_speed_ps, h2_speed_ps, tb_m_speed, tb_m_noisy, tb_m_cost, f_p_obj_tc, f_l_obj_tc, f_m_noisy, f_drop_obj_pen, d_pen, obs_h_wait,
          ...
          sort_traj, dec_mlp_layer_size, cen_mlp_layer_size, rnn_layer_num, dec_rnn_h_size, cen_rnn_h_size, 
          eval_freq, run_id, run_time, resume, save_ckpt, save_ckpt_time, log, seed, save_dir, device, **kwargs):
    ...
    if env_id.startswith('OSD') or env_id.startswith('BP'):
        ...
        env_cen = gym.make(env_id, **env_params)
        env_dec = gym.make(env_id, **env_params)
    else:
        TASKLIST = [
                "tomato salad", 
                "lettuce salad", 
        ...
        env_dec = gym.make(env_id, **env_params)
        if env_id.find("MA") != -1:
            env_cen = MacEnvWrapper(env_cen)
            env_dec = MacEnvWrapper(env_dec)
    ...
    # create team
    team = Team_RNN((env_cen, env_dec), env_terminate_step, n_env, (memory_cen, memory_dec), env_dec.n_agent, QLearn_squ_dec_cen[l_mode], h_stable_at,
            save_dir=save_dir, nn_model_params=model_params, eval_freq=eval_freq, seed=seed, log=log, **hyper_params)
    ...
    while team.episode_count < total_epi:
        ...
        if team.episode_count % eval_freq == 0 and team.envs_runner_dec.step_count[0] == 0:
        ...
        if (time.time()-t) // 3600 >= save_ckpt_time and save_ckpt:
            save_check_point(team.envs_runner_dec, team.envs_runner_cen, team.agents, team.cen_controller, step, team.episode_count, team.hysteretic, team.epsilon, save_dir, team.memory_cen, team.memory_dec, run_id, team.TEST_PERFORM) 
            save_ckpt_time *= 2
    ...
    if team.episode_count == total_epi and save_ckpt:
        save_check_point(team.envs_runner_dec, team.envs_runner_cen, team.agents, team.cen_controller, step, team.episode_count, team.hysteretic, team.epsilon, save_dir, team.memory_cen, team.memory_dec, run_id, team.TEST_PERFORM) 
        print('[{}]run, finish entire training ... '.format(run_id), flush=True)

    team.envs_runner_cen.close()
    team.envs_runner_dec.close()
...
def main():
    ...
    parser.add_argument('--dec_mlp_layer_size', action='store',         type=int, nargs='+', default=[32,32],       help='MLP layer dimension of decentralized policy-net')
    parser.add_argument('--cen_mlp_layer_size', action='store',         type=int, nargs='+', default=[32,32],       help='MLP layer dimension of decentralized policy-net')
    parser.add_argument('--rnn_layer_num',      action='store',         type=int,            default=1,             help='Number of RNN layers')
    parser.add_argument('--dec_rnn_h_size',     action='store',         type=int,            default=32,            help='RNN hidden layer dimension of decentralized policy-net')
    parser.add_argument('--cen_rnn_h_size',     action='store',         type=int,            default=64,            help='RNN hidden layer dimension of centralized policy-net')
    ...
    parser.add_argument('--log',                action='store_true',                                                help='Whether log or not')
    parser.add_argument('--run_id',             action='store',         type=int,            default=0,             help='Index of a run')
    parser.add_argument('--run_time',           action='store',         type=int,            default=20,            help='The number of hrs to run')
