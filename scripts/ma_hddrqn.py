...
from macro_marl.MA_hddrqn.team import Team_RNN
from macro_marl.MA_hddrqn.utils.memory import ReplayMemory_rand, ReplayMemory_epi
from macro_marl.MA_hddrqn.utils.utils import save_check_point
...
def train(env_id, obs_one_hot, target_flick_prob, agent_trans_noise, target_rand_move, 
          ...
          sort_traj, mlp_layer_size, rnn, rnn_layer_num, rnn_h_size, lstm, 
    ...
    # create team
    team = Team_RNN(env, env_terminate_step, n_env, memory, env.n_agent, QLearn_squ, h_stable_at,
            save_dir=save_dir, nn_model_params=model_params, eval_freq=eval_freq, seed=seed, writer=train_writer, log=log, **hyper_params)
    ...
    while team.episode_count < total_epi:
        ...
        if (time.time()-t) // 3600 >= save_ckpt_time and save_ckpt:
            save_check_point(team.envs_runner, team.agents, step, team.episode_count, team.hysteretic, team.epsilon, save_dir, team.memory, run_id, team.TEST_PERFORM) 
